            ...
            ${this.state.videos.get().map(v => {
                ...
                <div class="video-item ${this.state.currentVideoPath.get() === v.path ? 'active' : ''}" @click=${() => this.state.selectVideo(v.path)}>
                    ...
                    <div class="tag-container">
                        ...
                        ${isProcessing ? html`<span class="tag" style="background:var(--sl-color-primary-100); color:var(--sl-color-primary-700)">${Math.round((job?.progress || 0) * 100)}%</span>` : ''}
                        <span class="tag">${formatSize(v.size)}</span>
                        ${v.duration ? html`<span class="tag">${formatDuration(v.duration)}</span>` : ''}
                    </div>
                    <div class="video-meta">
                        <span>Video</span>
                         ${isProcessing ? html`<sl-spinner style="font-size: 1rem; --track-width: 2px;"></sl-spinner>` :
                            html`<sl-icon-button name="trash" class="delete-btn" label="Delete" @click=${(e: Event) => { e.stopPropagation(); this.state.deleteVideo(v.path); }}></sl-icon-button>`}
                    </div>
                </div>
            `})}
